...
<body>
	<div id="container ">
		...
		<div class="content">
			<div class="forms">
				<form:form action="${p.id }" method="POST" commandName="print"
					role="form">
					...
					<div class="col-sm-10 ">
								...
								<form:select path="user.id" class="form-control">
									<c:forEach items="${users}" var="user">
										<option value="${user.id}"
											${user.id == p.user.id ? 'selected="selected"' : '' }><c:out
												value="${p.user.completeName}" /></option>
									</c:forEach>
								</form:select>
						...
						<div class="col-sm-10 ">
							<div class="form-group form-group-lg">
								...
								<form:select path="pedigree.id" cssClass="form-control">
									<c:forEach items="${dogs}" var="dog">
										<option value="${dog.id}"
											${dog.id == p.id ? 'selected="selected"' : '' }><c:out
												value="${p.pedigree.name}" /></option>
									</c:forEach>
								</form:select>
							</div>
						</div>
						...
						<div class="col-sm-10">
							<div class="form-group">
								...
								<form:input path="dateIssue" value="${dateIssue}" type="text"
									class="form-control datepicker" placeholder="yyyy-MM-dd" required="required" />
							</div>
						</div>
					...
					<div class="col-sm-2 ">
						<div class="adSide">
							<a href="/PPCReg/print/ads/${adSide.id}"><img
...
<div class="adsFooter">
	<center>
		<a href="/PPCReg/print/ads/${adsFooter.id}"><img
